...
impl AppComponent for SlangExample {
    fn new(ctx: &mut CenContext) -> Self {
        ...
        let pipeline = ctx.create_pipeline(ComputePipelineConfig {
            shader_source: "examples/slang/shader.slang".into(),
            descriptor_set_layouts: vec![descriptorset.clone()],
            ..Default::default()
        }).expect("Failed to create Slang pipeline");

        Self { image, pipeline, _descriptorset: descriptorset }
    }

    fn window_event(&mut self, _: WindowEvent) {}
}

impl RenderComponent for SlangExample {
    fn render(&mut self, ctx: &mut CenContext) {
        ...
        ctx.command_buffer.bind_push_descriptor(compute, 0, &[write_descriptor_set]);
        ...
        ctx.command_buffer.clear_color_image(swapchain_image, vk::ImageLayout::TRANSFER_DST_OPTIMAL, [0.0, 0.0, 0.0, 1.0]);
...
impl GuiComponent for SlangExample {
    fn gui(&mut self, _: &mut GuiContext, _: &Context) {}
}

fn main() {
    Cen::<SlangExample>::run(AppConfig::default());
}
